...
fun main() {
    ...
    val source = "280dc9e47f3352c307f6d894ee8d534313429a79c1d8a6021f8a8eabca919cfb685a0d468973625e757490daa981ea6b\n" +
    ...
    val baseRowIndex = 17
    val guess = "With this regard their Currents turn awry,"

    val baseRow = rows[baseRowIndex]

    for (i in rows.indices) {
        val row = rows[i]
        val intermediate = if (row.size > baseRow.size) baseRow xor row else row xor baseRow

        val guessBytes = guess.toByteArray()
        val res = if (guessBytes.size > intermediate.size) intermediate xor guessBytes else guessBytes xor intermediate
        println("#$i " + res.joinToString(separator = "") {
...
infix fun ByteArray.xor(that: ByteArray) = mapIndexed { index, c ->
    that[index].xor(c)
}.toByteArray()